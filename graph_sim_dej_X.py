...
class ModelInference:
    ...
    def train_model(self):
        ...
        for epoch in range(self.epoch):
            ...
            if accuary_train >= 0.99 or (epoch >= 100 and epoch % 10 == 0 and accuary_train >= 0.98):
    ...
    def test_val_model(self, mode):
        ...
        for batch in test_loader:
            ...
            # 从中拿到不同的top top-1 top-2 top-3 

            # preds2 = torch.topk(outputs, k=2, dim=1)
            # accuary_all_num2 += torch.sum(preds2 == labels)

            # values2, indices2 = preds2

            # Convert boolean tensor to float tensor
            # accuary_all_num2 += torch.sum((indices2 == labels).float())

            # Similarly, for preds3



            # preds3 = torch.topk(outputs, k=3, dim=1)
            # values3, indices3 = preds3
            # accuary_all_num3 += torch.sum((indices3 == labels).float())
    ...
    def judge_graph_class_ac(self, mode, save_error_excel=False, all_test=False):
        ...
        if save_error_excel:
            ...
            if all_test:
                return df
        # logging.error("mode:{} top1_a:{} top2_a:{} top3_a:{}".format(mode, top1_a, top2_a, top3_a))
...
if __name__ == '__main__':
    # # get_error_summary(did=2)
    minf = ModelInference()
    # # # minf.generate_labeled_data()
    minf.train_model()
    # # path = "E:\code\python\\neo4j_aliyun\kb_algorithm\data\graph_sim_model_parameters\sock_shop\\20200422-082211_amaxD2_n30step10datasetraw"
    # path = "/data/sudong/code/graphsim_part/kb_algorithm/data/graph_sim_model_parameters/sock_shop/20200422-113035_amaxD2_n100step20datasetraw"
    # path = "/data/sudong/code/graphsim_part/kb_algorithm/data/graph_sim_model_parameters/train_ticket/20200416-131444_DESKTOP-8FNC2N9D1_n100step20repeatposauto"
    # minf.load_model(model_saved_dir=path)
    # minf.judge_graph_class_ac(mode="train", save_error_excel=False)
    # minf.judge_graph_class_ac(mode="test", save_error_excel=False)
    # minf.judge_graph_class_ac(mode="val", save_error_excel=False)
    # minf.test_val_model(mode="train")
    # minf.test_val_model(mode="test")
    # minf.test_val_model(mode="val")
    # # logging.info("done!")
